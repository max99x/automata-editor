...
class ChoiceWindow(object):
  def __init__(self, master, choices, title='Selection', text='Select item:', selected=0):
    ...
    self._list.pack(side = 'top', fill = 'both', expand=True, after=self._prompt)
    ...
    self._cancel = tk.Button(self._window, text = 'Cancel', command=self.close)
...
def askForOpen(master, title, type=('Finite State Machine', '*.fsm')):
  return tkFileDialog.askopenfilename(parent=master, title=title, filetypes=[type, ('All Files', '*.*')])

def askForSave(master, title, type=('Finite State Machine', '*.fsm')):
  return tkFileDialog.asksaveasfilename(parent=master, title=title, filetypes=[type, ('All Files', '*.*')], defaultextension=type[1])

def askYesNo(master, title, text):
  return tkMessageBox.askyesno(title=title, message=text, parent=master)

def messageBox(master, title, text, icon='warning'):
  if icon == 'warning':
    tkMessageBox.showwarning(title=title, message=text, parent=master)
  elif icon == 'info':
    tkMessageBox.showinfo(title=title, message=text, parent=master)
  else:
