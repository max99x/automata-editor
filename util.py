  ...
  if type(validType) is type:
    def setMethod(self, val):
      if type(val) == validType:
        ...
        raise AutomataError, '%s must be a %s, not %s.' % (attr.strip('_'), validType.__name__, type(val).__name__)
  else:
    def setMethod(self, val):
      if validType(val):
        setattr(self, attr, val)
      else:
        raise AutomataError, 'Attempted to set an invalid %s.' % attr.strip('_')
  return setMethod
  ...
  if colltype:
    def setMethod(self, val):
      if all([type(i) == validType for i in val]):
        ...
        raise AutomataError, '%s must contain only items of type: %s.' % (attr.strip('_'), validType.__name__)
  else:
    def setMethod(self, val):
  ...
  if type(validType) is type:
    def addMethod(self, item):
      if type(item) is validType:
        ...
        raise AutomataError, '%s must contain only items of type: %s, not %s.' % (attr.strip('_'), validType.__name__, type(item).__name__)
  else:
    def addMethod(self, item):
      if validType(item):
        getattr(self, attr).add(item)
  ...
  def remItemMethod(self, which):
    try:
      if type(which) is validType:
        ...
        raise AutomataError, '%s contains only items of type %s. Attempted to remove a %s' % (attr.strip('_'), validType.__name__, type(val).__name__)
    except:
      raise AutomataError, 'Attempted to remove a non-existant item from %s.' % attr.strip('_')
